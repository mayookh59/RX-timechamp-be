"""TrackMe Production Tracking Agent — Windows Edition.

Hardened for production deployment:
  - Single-instance lock via named mutex (prevents duplicate processes)
  - Watchdog thread restarts crashed worker threads
  - Rotating file logs (5 MB x 5 backups)
  - Graceful shutdown on SIGINT/SIGTERM/Windows console-close events
...
Tracks: active window, app usage, idle state, browser URLs, screenshots.
Syncs every 60 s to the TrackMe backend; buffers locally during outages.
"""
...
# ── Constants ────────────────────────────────────────────────────────
AGENT_VERSION = "2.0.0-python"

_PROGRAMDATA = os.environ.get("PROGRAMDATA", "C:\\ProgramData")
_DATA_DIR = os.path.join(_PROGRAMDATA, "TrackMe")

CONFIG_PATH = os.path.join(_DATA_DIR, "config.json")
DB_PATH = os.path.join(_DATA_DIR, "trackme_agent.db")
LOG_PATH = os.path.join(_DATA_DIR, "agent.log")
LOCK_PATH = os.path.join(_DATA_DIR, "agent.lock")

DEFAULT_CONFIG: dict[str, Any] = {
    # Production default — gets overridden by config.json written by the installer.
    # Replace this with your deployed backend URL (Railway / your domain).
    "api_base_url": "https://trackme.example.com/api/v1",
    "user_email": "admin@trackme.com",
...
# Sync-failure backoff: starts at 5 s, doubles each retry, capped at 5 min.
_BACKOFF_INITIAL = 5
_BACKOFF_MAX = 300

# Watchdog cadence — how often it checks worker liveness
_WATCHDOG_INTERVAL = 30

# DB maintenance cadence
_MAINT_INTERVAL = 3600  # hourly


# ── Configuration ────────────────────────────────────────────────────
def load_config() -> dict[str, Any]:
    """Load config from disk, falling back to defaults; never raises."""
    ...
    if os.path.exists(CONFIG_PATH):
        try:
            ...
            # Cannot use the logger yet (it's set up later); print a banner.
...
_mutex_handle = None  # keep alive at module scope so it isn't GC'd


def acquire_singleton_lock() -> bool:
    """Acquire a global named mutex so only one agent process runs.

    Returns True if this process is the sole owner, False if another agent
    is already running. The mutex is released automatically when the process
    exits (Windows kernel cleans it up).
    """
    global _mutex_handle
    try:
        kernel32 = ctypes.windll.kernel32
        ERROR_ALREADY_EXISTS = 183
        # Global\\ prefix makes it visible across user sessions
        mutex_name = "Global\\TrackMeAgentSingletonMutex"
        _mutex_handle = kernel32.CreateMutexW(None, False, mutex_name)
        if not _mutex_handle:
            return True  # mutex creation failed — fall through and let it run
        if kernel32.GetLastError() == ERROR_ALREADY_EXISTS:
            return False
        return True
    except Exception:
        # If anything goes wrong with mutex acquisition, allow the agent to run
        # (better to have a duplicate than to fail to start).
        return True
...
def get_idle_seconds() -> float:
    """Return seconds since the last keyboard/mouse input. 0 on error."""
    try:
        lii = _LASTINPUTINFO()
...
def get_foreground_window_info() -> tuple[str, str, int]:
    """Return (window_title, process_name, pid) of the foreground window."""
    try:
...
def _get_conn() -> sqlite3.Connection:
    """Return a new SQLite connection with WAL + busy-timeout."""
...
def init_db() -> None:
    ...
    Migrations are forward-only and safe to run on every startup. They use
    PRAGMA table_info to detect missing columns added in newer versions
    (e.g. duration_sec on activity_sessions added in v2.0.0).
    """
...
def _run_migrations(conn: sqlite3.Connection) -> None:
    """Apply forward-only schema migrations to bring older DBs up to date.

    Uses PRAGMA table_info to detect missing columns. Safe to run repeatedly.
    """
    ...
    for table, col, sql in migrations:
        try:
            ...
            # Table might not exist yet (shouldn't happen post-create) — skip
...
def register_device(config: dict[str, Any]) -> tuple[str, str]:
    """Register this device; persist + return (api_key, device_id).

    On any failure returns ('', '') — caller should retry later.
    """
    user_email = kv_get("user_email") or config.get("user_email", "admin@trackme.com")
    try:
        resp = requests.post(
...
def load_credentials() -> tuple[str, str]:
    """Load saved (api_key, device_id) from local DB."""
    return (kv_get("api_key") or "", kv_get("device_id") or "")


# ── Activity tracker ─────────────────────────────────────────────────
class ActivityTracker(threading.Thread):
    """Polls active window + idle state every 2 seconds."""

    SYSTEM_PROCESSES = {"LockApp.exe", "LogonUI.exe", "ScreenSaverUI.exe"}
    BROWSER_PROCESSES = {"chrome.exe", "msedge.exe", "firefox.exe", "brave.exe", "opera.exe", "vivaldi.exe"}
    CHECKPOINT_INTERVAL = 300  # save partial sessions every 5 min
    ...
    def heartbeat(self) -> float:
        """Watchdog uses this to check liveness."""
    ...
    def run(self) -> None:
        ...
        try:
            while not self.shutdown.is_set():
                ...
                # Sleep in 0.5 s slices so shutdown is responsive
                for _ in range(4):
    ...
    def _tick(self) -> None:
        ...
        # Foreground app + URL — only when active
        if new_state == "active":
            ...
            if proc and proc not in self.SYSTEM_PROCESSES:
                ...
                if proc.lower() in self.BROWSER_PROCESSES and title:
    ...
    @staticmethod
    def _extract_domain(title: str) -> tuple[str, str]:
        """Strip browser suffix and look for a domain in the title."""
    ...
    def _track_url(self, proc: str, title: str) -> None:
        ...
        if page_title != self.current_url_title:
            ...
            self.current_url_browser = proc.replace(".exe", "")
...
class SyncEngine(threading.Thread):
    """Sends buffered tracking data to the backend.

    Backoff strategy:
      - On any sync HTTP error, backoff = min(backoff * 2, 5 min) + jitter
      - On success, backoff resets to interval
    """
    ...
    def run(self) -> None:
        ...
        while not self.shutdown.is_set():
            ...
            try:
                ...
                # 401 — credentials invalid, force re-register on next cycle
                logging.warning("Credentials lost (401); clearing api_key and device_id")
                ...
                self.backoff = min(self.backoff * 2, _BACKOFF_MAX)
            # Sleep with jitter; break on shutdown
    ...
    def _sync_sessions(self) -> None:
        ...
        if self._check_response(resp, "sessions"):
            with _db_lock:
                ...
                try:
                    conn.executemany(
                        "UPDATE activity_sessions SET synced=1 WHERE id=?",
                        [(r[0],) for r in rows],
                    )
                    conn.commit()
                finally:
    ...
    def _sync_screenshots(self) -> None:
        # Keep batch small — base64 payloads are large
        with _db_lock:
            conn = _get_conn()
            try:
                rows = conn.execute(
    ...
    def _send_heartbeat(self) -> None:
        ...
        # queue depth = sum of unsynced rows across tables
        try:
            conn = _get_conn()
            try:
                qd = (
            ...
            qd = 0
        # Last ~10 KB of log so admins can view remote agent state via the
        # /agents/{device_id}/diagnostics endpoint.
        log_tail = ""
        try:
...
class DBMaintenance(threading.Thread):
    """Periodically removes synced rows older than retention_days and vacuums."""
    ...
    def run(self) -> None:
        ...
        # Initial delay so it doesn't run during startup churn — but keep
        # the heartbeat fresh so the watchdog doesn't flag us.
        for _ in range(120):
            if self.shutdown.is_set():
                return
            self.heartbeat_at = time.time()
        ...
        while not self.shutdown.is_set():
            ...
            # Sleep between maintenance cycles, updating heartbeat each iter
            slept = 0
    ...
    def _maintain(self) -> None:
        ...
        with _db_lock:
            conn = _get_conn()
            try:
                for tbl, ts_col in [("activity_sessions", "start_time"), ("app_usage", "start_time"),
                                    ("url_visits", "visit_time"), ("screenshots", "captured_at")]:
                    cur = conn.execute(
                        f"DELETE FROM {tbl} WHERE synced=1 AND {ts_col} < ?", (cutoff,)
                    )
                    deleted[tbl] = cur.rowcount
                conn.commit()
                # Cap DB size — if over limit, also drop OLDEST synced screenshots
...
class Watchdog(threading.Thread):
    """Restarts worker threads if they stop heartbeating for >2 min."""
        ...
        while not self.shutdown.is_set():
            ...
            for name, thread in list(self.workers.items()):
                if not thread.is_alive():
                    ...
                    new.start()
                    continue
                hb_fn = getattr(thread, "heartbeat", None)
                if callable(hb_fn):
                    hb_age = now - hb_fn()
                    if hb_age > self.STALE_AFTER:
                        logging.error("Watchdog: %s heartbeat stale (%ds) — flagging", name, int(hb_age))
        logging.info("Watchdog stopped")


# ── System tray ──────────────────────────────────────────────────────
def run_tray_or_block(workers: dict[str, threading.Thread], shutdown: threading.Event) -> None:
    """Run the pystray icon (if available) or block until shutdown."""
    try:
...
def setup_logging(config: dict[str, Any]) -> None:
    ...
    root.setLevel(level)
    # Clear any existing handlers (in case main is re-entered)
    root.handlers = []
    for h in handlers:
        root.addHandler(h)
...
def install_signal_handlers(shutdown: threading.Event) -> None:
    ...
    for sig in (signal.SIGINT, signal.SIGTERM):
        try:
            ...
            # SIGTERM might not be available in some embedded contexts; non-fatal
            pass

    # Windows console close (Ctrl-C is handled by SIGINT above)
    try:
        kernel32 = ctypes.windll.kernel32
        HANDLER_ROUTINE = ctypes.WINFUNCTYPE(ctypes.c_bool, ctypes.c_uint)

        def _ctrl_handler(_event):
            logging.info("Console event received — shutting down")
            shutdown.set()
            return True

        global _ctrl_routine
        _ctrl_routine = HANDLER_ROUTINE(_ctrl_handler)
        kernel32.SetConsoleCtrlHandler(_ctrl_routine, True)
    except Exception:
        pass
...
def run_diagnostics(config: dict[str, Any]) -> int:
    ...
    Use this to debug "why is the agent misbehaving" in one command.
    Output is plain text, intended to be copy-pasted into a bug report.
    """
    sep = "=" * 70
    print(sep)
    print("  TrackMe Agent Diagnostic Report")
    print(sep)
    ...
    try:
        ...
        kernel32 = ctypes.windll.kernel32
        ERROR_ALREADY_EXISTS = 183
        h = kernel32.CreateMutexW(None, False, "Global\\TrackMeAgentSingletonMutex")
        already = kernel32.GetLastError() == ERROR_ALREADY_EXISTS
        if h:
            kernel32.ReleaseMutex(h)
            kernel32.CloseHandle(h)
        print(f"  Another agent currently running: {'YES' if already else 'no'}")
    except Exception as e:
        print(f"  Mutex check error: {e}")
    ...
    try:
        candidates = [p for p in psutil.process_iter(["pid", "name", "cmdline"])
                      ...
                      and p.info["cmdline"]
                      and any("trackme_agent" in str(arg) for arg in p.info["cmdline"])]
        if candidates:
            for p in candidates:
                print(f"  PID {p.info['pid']}: {p.info['name']} {' '.join(p.info['cmdline'][1:])}")
        else:
            print("  No trackme_agent processes found")
    except Exception as e:
    ...
    try:
        ...
        # HTTP probe
        try:
            r = requests.get(f"{api_base}/../docs" if api_base.endswith("/api/v1") else api_base, timeout=5)
            print(f"  HTTP /docs:  {r.status_code}")
        except Exception as e:
            print(f"  HTTP probe:  FAILED ({e})")
    except Exception as e:
        print(f"  Network test error: {e}")

    print(f"\n{sep}\n  Last 30 log lines (most recent at bottom)\n{sep}")
    try:
        if os.path.exists(LOG_PATH):
            ...
            print("  (agent.log not present — agent has never run from this script)")
    except Exception as e:
...
# ── Main ─────────────────────────────────────────────────────────────
def main() -> int:
    parser = argparse.ArgumentParser(description="TrackMe Agent (Windows)")
    parser.add_argument("--once", action="store_true", help="Run a single sync cycle and exit (for testing)")
    parser.add_argument("--diagnose", action="store_true",
                        help="Print full diagnostic report (config, DB stats, network, recent log) and exit")
    ...
    if args.diagnose:
        # Don't take the singleton lock — this is read-only
        try:
            init_db()  # ensure tables exist for the read
        except Exception:
            pass
    ...
    logging.info("Python: %s | Data dir: %s", sys.version.split()[0], _DATA_DIR)
    logging.info("API: %s", config["api_base_url"])
    logging.info("Sync interval: %ds | Idle threshold: %ds | Screenshot interval: %ds",
    ...
    # Try a one-shot register so credentials are present at startup; sync engine
    # will retry on its own if this fails.
    api_key, device_id = load_credentials()
    ...
    if args.once:
        # Test mode: run one sync cycle and quit
        SyncEngine(config, shutdown)._cycle()  # type: ignore[attr-defined]
        return 0

    # Build workers + factories so the watchdog can recreate them on death
    factories = {
    ...
    workers: dict[str, threading.Thread] = {name: f() for name, f in factories.items()}
    for w in workers.values():
        w.start()

    watchdog = Watchdog(workers, factories, shutdown)
    watchdog.start()

    # Block in tray icon (or headless) — returns when the user picks Quit
    try:
        run_tray_or_block(workers, shutdown)
